...
function createAttachmentRow(attachment) {
    ...
    noteInput.type = "text";
    noteInput.placeholder = "Attachment note";
...
function readAttachmentsFromForm() {
    return Array.from(offerAttachmentsInput.querySelectorAll(".offer-attachment-row")).map((row) => {
        ...
        return {
            cardId: dsNormalizeCardId(fields[0].value),
            cardInstanceId: dsNormalizeCardId(fields[1].value || ""),
            note: String(fields[2].value || "").trim(),
        };
...
function validateOffersWithRelations(offers) {
    ...
    const errors = [];
    const categoryKeys = new Set(Object.keys(dsCategoryDefs || {}));
